...
### ⭐ 인터랙티브 게이트 진행 규칙 (강의 녹화 기본값)

> 사용자가 "하나씩 단계별로 물어보며 진행" 을 원할 때의 진행 방식. 클릭 하나하나를 실시간으로 따라가는 느낌을 만든다.

1. **한 MCP = 한 게이트** — 1번 → 12번 순서. 한 MCP(①~④)가 끝나면 **멈추고 "다음" 확인을 받은 뒤에만** 다음 MCP 로 넘어간다. 12개를 한 메시지에 몰아 출력하지 않는다.
2. **②발급 절차는 "한 클릭(한 단계)씩"** — 발급 단계를 한꺼번에 나열하지 않는다. 한 화면/한 클릭만 보여주고 → "됐어/다음" 받으면 → 다음 클릭 공개.
3. **실제 홈페이지·콘솔 링크를 매 단계 포함** — 클릭할 정확한 URL 을 클릭 가능한 링크로 제시.
4. **③설치 명령은 직접 실행** — 코드블록으로 "보여주기만" 하지 않고 터미널에서 실제로 실행한다.
5. **기존 키는 절대 덮어쓰지 않는다** — 본체 프로젝트는 이미 12개가 작동 중이므로 키를 placeholder 로 덮는 `sed` 는 금지. 이미 설정된 항목은 **기존 `.env` 를 살린 채 검증용 실명령**(`set -a; source .env` 후 헬스체크/버전확인)으로 실행. 처음 설치하는 학습자는 같은 자리에서 본인 키가 실제로 쓰인다.

> 실측 경로: 본체(라이브)=`…/Desktop/0.마케터를 위한 클로드코드/marketing-os` (여기에 `.env`+`.mcp.json` 살아있음 · `claude mcp list` 는 이 폴더에서) · 클론(문서 전용)=`…/Desktop/MCP 12종 설치 /marketing-os-mcp-setup`.
